    ...
    <x-slot name="header">
        <div class="flex ">
            ...
            <div class="flex-none w-24 h-16 ">
                <x-jet-nav-link href="{{ route('task.create') }}" class="" active>
                    Add Task
                </x-jet-nav-link>
            </div>
          </div>
    ...
    <div class="py-6">
        <div class="justify-center mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="block m-6 overflow-x-auto">
                @include('components.filter', ['rota' => "task.index"])
                <br>
                    ...
                    <tbody>
                        @foreach ($tasks as $task)
                            <tr class="w-full font-light text-gray-200 whitespace-no-wrap bg-gray-600 border border-b-0">
                                <td class="px-4 py-4">{{ $task->task }}</td>
                                <td class="px-4 py-4">{{ $task->description }}</td>
                                <td class="px-4 py-4">
                                    @if ($task->status == 'todo')
                                        <span class="px-2 py-1 text-sm text-white bg-blue-500 rounded-full">{{ $task->status }}</span>
                                    @elseif($task->status == 'doing')
                                        <span class="px-2 py-1 text-sm text-white bg-yellow-500 rounded-full">{{ $task->status }}</span>
                                    @else
                                        <span class="px-2 py-1 text-sm text-white bg-green-500 rounded-full">{{ $task->status }}</span>
                                    @endif
                                ...
                                <td class="text-center w-32">
                                    ...
                                    <a href="{{ route('task.edit', ['id' => $task->id]) }}"><span ><i class=" text-yellow-400  hover:text-yellow-700 fas fa-edit fa-2x"></i></span></a>
